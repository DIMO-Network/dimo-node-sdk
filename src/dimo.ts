...
import { ClientConfigDimo, ContractType, DIMO_APIs, ENVIRONMENT } from "./utils/types";
import { CHAIN_ABI_MAPPING, ENV_TO_API_MAPPING, api_mapping_by_env } from "./utils/constants";
...
export class DIMO {
  ...
  constructor(config: ClientConfigDimo) {
    const env = api_mapping_by_env.get(config.environment) || ENVIRONMENT.DEV;
    const apiMapping = ENV_TO_API_MAPPING[env];
    const abiMapping = CHAIN_ABI_MAPPING[env];
     ...
     * Set up Web3 Client
     */
    this.web3 = new DimoWeb3Client(env);
  }
